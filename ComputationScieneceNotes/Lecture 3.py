...
# Command line code
# ls # makes a directory
# mkdir classCode # names the directory
# cd ClassCodes/ # looks into the directory
# ls # makes a new file
# atom # opens atom(IDE)



# Bisection as programmed in Lecture 3
# Input function f, starting points a0 and b0
#   also need tolerances eps_x, eps_f and
#   max number of iterations k_max
    ...
    if (f(a0)*f(b0) < 0):         
        ...
        for k in range(k_max):
            ...
            if (max_error < eps_x) and (res < eps_f):     # a break function if we reach the max error while in a loop
                conv = 1                # When it converges we send back one
                break
